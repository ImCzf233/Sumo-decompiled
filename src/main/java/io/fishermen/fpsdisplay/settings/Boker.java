...
public class Boker extends GuiSettings
{
    ...
    public Boker() {
        super(GuiSettings.a(new char[] { 'A', 'i', 'm' }), "", c4.a, 0, 0);
        Boker.d = new CommandSettings(GuiSettings.a(new char[] { 'S', 'p', 'e', 'e', 'd' }), 35.0, 1.0, 75.0, 1.0);
        Boker.f = new CommandSettings(GuiSettings.a(new char[] { 'F', 'O', 'V' }), 70.0, 30.0, 180.0, 1.0);
    ...
    public Entity gg() {
        ...
        for (final Object o : Boker.mc.theWorld.loadedEntityList) {
            ...
            if (ent.isEntityAlive() && !ent.isInvisible() && ent != Boker.mc.thePlayer && Boker.mc.thePlayer.getDistanceToEntity(ent) < 6.0f && this.vvczcvx(ent) && ent instanceof EntityLivingBase && Boker.mc.thePlayer.canEntityBeSeen(ent) && bvc(ent, (float)f)) {
                e = ent;
                f = (int)vcx(ent);
            }
        }
    ...
    public boolean vvczcvx(final Entity e) {
        return !(e instanceof EntityLivingBase) || ((EntityLivingBase)e).getTeam() == null || !((EntityLivingBase)e).isOnSameTeam((EntityLivingBase)Boker.mc.thePlayer) || !Boker.teams.i();
    }
